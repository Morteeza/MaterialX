...
void MslResourceBindingContext::emitDirectives(GenContext& context, ShaderStage& stage)
{
}
...
void MslResourceBindingContext::emitResourceBindings(GenContext& context, const VariableBlock& uniforms, ShaderStage& stage)
{
    ...
    if (hasValueUniforms)
    {
        generator.emitLine("struct " + uniforms.getName() /*+ "_" + stage.getName()*/,
                           stage, false);
    ...
    //    if (uniform->getType() == Type::FILENAME)
    //    {
    //        //generator.emitString(syntax.getUniformQualifier() + " ", stage);
    //        generator.emitVariableDeclaration(uniform, EMPTY_STRING, context, stage, false);
