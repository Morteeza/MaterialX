...
MslProgram::MslProgram() :
    _shader(nullptr),
    _vertexArray(UNDEFINED_METAL_RESOURCE_ID),
...
id<MTLRenderPipelineState> MslProgram::build(id<MTLDevice> device, MetalFramebufferPtr framebuffer)
{
    ...
    {
        ImagePtr imgColor = framebuffer->getColorImage();
        
        MTLDataType    imgColorDataType;
        MTLPixelFormat imgColorPixelFormat;
        MetalTextureHandler::mapTextureFormatToMetal(imgColor->getBaseType(), imgColor->getChannelCount(), true, imgColorDataType, imgColorPixelFormat);
        ...
        psoDesc.colorAttachments[0].pixelFormat =  imgColorPixelFormat;
...
void MslProgram::bindInputs(id<MTLRenderCommandEncoder> renderCmdEncoder,
                                 CameraPtr cam,
...
void MslProgram::unbindGeometry()
{
    ...
    _vertexArray = MslProgram::UNDEFINED_METAL_RESOURCE_ID;
}

ImagePtr MslProgram::bindTexture(id<MTLRenderCommandEncoder> renderCmdEncoder,
                                 unsigned int uniformType, int uniformLocation,
                                 ...
                                 const ImageSamplingProperties& samplingProperties)
{
    ...
    if (static_cast<MetalTextureHandler*>(imageHandler.get())->bindImage(_device, renderCmdEncoder, uniformLocation, image, samplingProperties))
    {
        return image;
    }
    return nullptr;
                              ...
                              ImageHandlerPtr imageHandler)
{
    for (MTLArgument *arg in _psoReflection.fragmentArguments)
    {
        if (arg.type == MTLArgumentTypeTexture)
        {
                ...
                for (const auto& env : envLights)
                {
                    ...
                    if(loc != std::string::npos && env.second)
                    {
                        ...
                        static_cast<MaterialX::MetalTextureHandler*>
                            (imageHandler.get())->bindImage(_device, renderCmdEncoder,
                                                            arg.index, env.second,
                                                            samplingProperties);
            ...
            if(!found)
            {
                id<MTLTexture> metalTexture =  static_cast<MetalTextureHandler*>(imageHandler.get())->getMTLTextureForImage(arg.index);
                if(metalTexture != nil)
                {
                    [renderCmdEncoder setFragmentTexture:metalTexture atIndex:arg.index];
                    
                    id<MTLSamplerState> metalSamplerState = static_cast<MetalTextureHandler*>(imageHandler.get())->getMTLSamplerStateForImage(arg.index);
                    
                    [renderCmdEncoder setFragmentSamplerState:metalSamplerState atIndex:arg.index];
            ...
            if(!found)
            {
                ...
                {
                    ImageSamplingProperties samplingProperties;
                    //samplingProperties.setProperties(uniform->first, uniform->second->value);
                    string fileName = uniform->second->value ? uniform->second->value->getValueString() : "";
                    bindTexture(renderCmdEncoder, 0, arg.index, fileName, imageHandler, samplingProperties);
                }
            }
        }
    }
...
void MslProgram::bindLighting(LightHandlerPtr lightHandler, ImageHandlerPtr imageHandler)
{
    ...
    if (albedoTable && hasUniform(HW::ALBEDO_TABLE))
    {
        ...
        {
            MetalTextureHandlerPtr textureHandler = std::static_pointer_cast<MetalTextureHandler>(imageHandler);
    ...
    if (_shader)
    {
        ...
        for (const auto& uniformMap : ps.getUniformBlocks())
        {
            ...
            for (size_t i = 0; i < uniforms.size(); ++i)
            {
                ...
                int resourceType = mapTypeToMetalType(v->getType());
...
{
    auto setCommonUniform = [](LightHandlerPtr lightHandler, CameraPtr cam, std::string uniformName, std::vector<unsigned char>& data, size_t offset) -> bool
    {
    ...
    auto setValue = [](MaterialX::ValuePtr value, std::vector<unsigned char>& data, size_t offset)
    {
        ...
        else if (value->getTypeString() == "matrix33")
        {
            ...
            memcpy((void*)&data[offset], &m, sizeof(Matrix33));
        }
