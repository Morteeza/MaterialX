...
void MslRenderer::initialize()
{
    if (!_initialized)
    {
        ...
        _framebuffer = MetalFramebuffer::create(_device, _width, _height, 4, _baseType);
...
void MslRenderer::renderTextureSpace(const Vector2& uvMin, const Vector2& uvMax)
{
    ...
    _program->bindTextures(rendercmdEncoder, _lightHandler, _imageHandler);
...
void MslRenderer::setSize(unsigned int width, unsigned int height)
{
    ...
    {
        _framebuffer = MetalFramebuffer::create(_device, width, height, 4, _baseType);
    }
    _width = width;
    _height = height;
}
...
void MslRenderer::render()
{
    ...
    try
    {
        ...
        if (_program)
        {
            ...
            _program->bind(renderCmdEncoder);
            _program->bindInputs(renderCmdEncoder, _camera, _geometryHandler, _imageHandler, _lightHandler);
