...
bool MetalTextureHandler::bindImage(id<MTLDevice> device, id<MTLRenderCommandEncoder> renderCmdEncoder, int textureUnit, ImagePtr image, const ImageSamplingProperties& samplingProperties)
{
    // Create renderer resources if needed.
    if (image->getResourceId() == MslProgram::UNDEFINED_METAL_RESOURCE_ID)
    {
    ...
    [renderCmdEncoder setFragmentSamplerState:getSamplerState(samplingProperties) atIndex:textureUnit];
...
bool MetalTextureHandler::createRenderResources(ImagePtr image, bool generateMipMaps)
{
    ...
    if (image->getResourceId() == MslProgram::UNDEFINED_METAL_RESOURCE_ID)
    {
        ...
        texDesc.usage = MTLStorageModePrivate | MTLTextureUsageShaderRead;
    ...
    id<MTLBuffer> buffer = [_device newBufferWithBytes:imageData length:sourceBytesPerImage options:MTLStorageModeShared];
    [blitCmdEncoder copyFromBuffer:buffer sourceOffset:0 sourceBytesPerRow:sourceBytesPerRow sourceBytesPerImage:sourceBytesPerImage sourceSize:MTLSizeMake(image->getWidth(), image->getHeight(), 1) toTexture:texture destinationSlice:0 destinationLevel:0 destinationOrigin:MTLOriginMake(0, 0, 0)];
...
int MetalTextureHandler::getNextAvailableTextureLocation()
{
    for (size_t i = 0; i < _boundTextureLocations.size(); i++)
    {
        if (_boundTextureLocations[i] == MslProgram::UNDEFINED_METAL_RESOURCE_ID)
        {
            return static_cast<int>(i);
        }
    }
