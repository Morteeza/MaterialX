...
class MX_RENDERMSL_API MetalTextureHandler : public ImageHandler
{
  public:
    ...
    void pushRenderCommandEncoder(id<MTLRenderCommandEncoder> renderCmdEncoder)
    { _renderCmdEncoderStack.push(renderCmdEncoder); }
    
    void popRenderCommandEncoder()
    { _renderCmdEncoderStack.pop(); }
    ...
    bool bindImage(id<MTLDevice> device, id<MTLRenderCommandEncoder> renderCmdEncoder, int textureUnit, ImagePtr image, const ImageSamplingProperties& samplingProperties) /*override*/;
  ...
  protected:
    ...
    std::stack<id<MTLRenderCommandEncoder>> _renderCmdEncoderStack;
};
